...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        output = []
        try:
            array.append(state.stack[-1])
        except:
            array.append('<NULL>')
        try:
            array.append(state.stack[-2])
        except:
            array.append('<NULL>')
        try:
            array.append(state.stack[-3])
        except:
            array.append('<NULL>')
        try:
            array.append(state.buffer[-1])
        except:
            array.append('<NULL>')
        try:
            array.append(state.buffer[-2])
        except:
            array.append('<NULL>')
        try:
            array.append(state.buffer[-3])
        except:
            array.append('<NULL>')
        ...
        for i in array:
            if i == 0:
                output.append(self.word_vocab.get("<ROOT>"))
            elif not i == '<NULL>':
                p = "<" + str(pos[i]) + ">"

                if words[i] in self.word_vocab:
                    output.append(self.word_vocab.get(words[i]))
                elif p in self.word_vocab:
                    output.append(self.word_vocab.get("<" + str(pos[i]) + ">"))
                else:
                    output.append(self.word_vocab.get('<UNK>'))
            else:
                output.append(self.word_vocab.get('<NULL>'))
    ...
    def get_output_representation(self, output_pair):
        # convert pairs & generate matrices
        if output_pair in self.output_labels:
            index = self.output_labels.get(output_pair)
            return(keras.utils.to_categorical(index,num_classes=91))
        return np.zeros(91)
